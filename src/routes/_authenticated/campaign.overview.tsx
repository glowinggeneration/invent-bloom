...
import { CalendarClock, CheckCircle2, Clock, Gauge, Loader2, Users } from "lucide-react";
...
function CampaignOverviewPage() {
  ...
  const isPending = accountsQuery.isPending || jobsQuery.isPending || queueQuery.isPending;
    ...
    <WorkspaceShell title="Create campaign" wide>
      ...
      {isPending && (
        <div className="mt-6 flex items-center gap-2 type-body text-muted-foreground">
          <Loader2 className="size-4 animate-spin" /> Loading your campaign activity…
        </div>
      )}
