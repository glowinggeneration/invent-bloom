 ...
 * source https://github.com/Ashutoshx7/VengeanceUI. Only change from the
 * original: framer-motion -> motion/react (this app's installed package,
 * same API). Already theme-adaptive via var(--foreground)/var(--background)
 * defaults, so no retinting needed - pass `accentColor` at the call site
 * for a brand-specific hover color.
 */
...
export type AnimatedTooltipVariant =
  "cora" | "smaug" | "dori" | "gram" | "indis" | "malva" | "sadoc";
  ...
  cora: {
    ...
    content: {
      ...
      animate: {
        y: 0,
        opacity: 1,
        transition: { duration: 0.3, delay: 0.25, ease: EASE_OUT_QUINT },
      },
  ...
  dori: {
    ...
    content: {
      ...
      animate: {
        y: 0,
        opacity: 1,
        transition: { duration: 0.3, delay: 0.1, ease: EASE_OUT_QUINT },
      },
      exit: { y: 20, opacity: 0, transition: { duration: 0.1, ease: EASE_IN } },
    },
    contentStyle: { marginBottom: "0.75em" },
