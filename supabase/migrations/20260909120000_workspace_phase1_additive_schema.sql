-- Multi-tenant SaaS conversion, Phase 1: additive schema only.
--
-- This migration adds the workspaces/workspace_members tables and a nullable
-- workspace_id column on every tenant-scoped table, backfilled to a single
-- "Workspace 1" row representing the current deployment's one existing
-- tenant. Nothing reads workspace_id yet - RLS policies, the
-- resolveWorkspaceId() helper, and application code are all unchanged by
-- this migration. That rework is Phase 2, done deliberately as a separate,
-- independently-reviewable step so a mistake there is caught against the
-- one existing tenant before any second tenant can exist.
--
-- workspace_settings.singleton / workspace_execution_state.singleton are
-- deliberately NOT touched here - dropping that primary key is a real
-- breaking change, not additive, and belongs at the start of Phase 2
-- immediately before those two tables' RLS gets rewritten.
...
-- Client read policy (members can read their own workspace row) is added in
-- Phase 2 alongside workspace_members' read path, so both land together and
-- get verified as one unit rather than half-wired now.
...
-- Same "read your own rows, no client writes" convention as user_roles
-- (see 20260830211556_...sql) - membership changes go through server code
-- (invite/accept/role-change server functions), never a direct client write.
...
-- 2. Backfill: one workspace for the current single-tenant deployment, every
-- existing user becomes a member. The existing hardcoded admin email
-- (src/lib/access.ts ADMIN_EMAIL) becomes that workspace's owner; everyone
-- else lands as a member. This does not change what anyone can access today
-- (RLS is unchanged by this migration) - it just gives the existing tenant a
-- real row to point workspace_id at once Phase 2 starts reading it.
...
-- 3. Nullable workspace_id on every tenant-scoped table, backfilled to
-- Workspace 1. NOT NULL and RLS enforcement are Phase 2 - this step only
-- grows the schema.
...
-- workspace_settings / workspace_execution_state: nullable workspace_id
-- added alongside the existing singleton PK, which is left untouched here
-- (see header comment). Once Phase 2 drops the singleton constraint and
-- makes workspace_id the primary key, this column already has the right
-- backfilled value.
...
-- Platform scaffolding tables (audit_log, rate_limit_hits, idempotency_keys,
-- ai_events) - added here too per the plan, before more call sites are
-- wired into them.
